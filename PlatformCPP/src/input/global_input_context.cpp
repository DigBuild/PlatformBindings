...
extern "C" {
	...
	DLLEXPORT void dbp_global_input_context_update(
		GlobalInputContext* instance
	)
	{
		instance->update();
	...
	DLLEXPORT void dbp_global_input_context_get_controller_state(
		const native_handle instance,
		void(*callback)(bool connected, uint8_t* buttonStates, uint32_t buttonCount, float* joystickStates, uint32_t joystickCount, uint8_t* hatStates, uint32_t hatCount)
	)
	{
		auto* controller = handle_cast<Controller>(instance);

		if (!controller->isConnected())
			return callback(false, nullptr, 0, nullptr, 0, nullptr, 0);
